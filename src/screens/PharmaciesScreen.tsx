...
import { pharmacies } from '../data/emergencyData';

type Props = {
  navigation: NativeStackNavigationProp<RootStackParamList, 'Pharmacies'>;
};

export default function PharmaciesScreen({ navigation }: Props) {
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={[styles.header, { backgroundColor: colors.headerBg }]}>
        <View style={[styles.headerCircle, { backgroundColor: isDark ? 'rgba(33,150,243,0.04)' : 'rgba(33,150,243,0.06)' }]} />
        ...
        <View style={[styles.headerIcon, { backgroundColor: '#2196F320' }]}>
          <Ionicons name="medkit" size={28} color="#2196F3" />
        </View>
        <Text style={[styles.headerTitle, { color: colors.text }]}>{t('pharmacies_title')}</Text>
      </View>
      ...
      <View style={styles.searchWrap}>
        <View style={[styles.searchBar, { backgroundColor: colors.card, borderColor: colors.border }]}>
          ...
          <TextInput
            ...
            placeholder={t('pharmacies_search')}
      ...
      <View style={[styles.devBanner, { backgroundColor: isDark ? '#1A1500' : '#FFFBEB' }]}>
        ...
        <Text style={[styles.devText, { color: isDark ? '#FCBF49' : '#92400E' }]}>{t('pharmacies_dev')}</Text>
      </View>
        ...
        <View style={[styles.listCard, { backgroundColor: colors.card, borderColor: colors.border, shadowColor: colors.shadowColor }]}>
          ...
          {filtered.map((pharmacy, index) => (
            <TouchableOpacity
              ...
              activeOpacity={0.6}
            >
              <View style={[styles.iconCircle, { backgroundColor: '#2196F318' }]}>
                <Ionicons name="medkit" size={20} color="#2196F3" />
              </View>
              <View style={styles.rowInfo}>
                <Text style={[styles.name, { color: colors.text }]} numberOfLines={1}>{pharmacy.name}</Text>
                <View style={styles.metaRow}>
                  {pharmacy.isOpen24h && (
                    <View style={[styles.badge, { backgroundColor: '#2196F315' }]}>
                      <Text style={[styles.badgeText, { color: '#2196F3' }]}>{t('pharmacies_24h')}</Text>
                    </View>
                  )}
                  <Ionicons name="location-outline" size={12} color={colors.textMuted} />
                  <Text style={[styles.city, { color: colors.textMuted }]}>{pharmacy.city}</Text>
                </View>
              </View>
              <View style={[styles.phoneIcon, { backgroundColor: '#2196F315' }]}>
